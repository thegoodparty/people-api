...
class Analyzer {
    ...
    analyzeType(type, location, optional = false, inputOutput = InputOutput.Neither, docString = undefined) {
        var _a, _b, _c;
        if (isSimpleType(type)) {
            ...
            prop.items = this.analyzeType(innerType, location, false /* optional */, inputOutput === InputOutput.Output ? inputOutput : InputOutput.Neither);
            if (docString) {
                prop.description = docString;
            }
            return prop;
            ...
            if (!nonUndefinedType) {
                throw new Error(`Expected exactly one type to not be undefined in '${this.checker.typeToString(type)}'`);
            }
